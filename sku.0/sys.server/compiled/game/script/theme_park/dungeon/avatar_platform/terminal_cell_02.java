...
public class terminal_cell_02 extends script.base_script
{
    public terminal_cell_02()
    {
    }
    ...
    public int OnObjectMenuSelect(obj_id self, obj_id player, int item) throws InterruptedException
    {
        if (item == menu_info_types.ITEM_USE)
        {
            ...
            obj_id target = getObjIdObjVar(structure, "avatar_platform.wke_prisoner_02");
            if (!hasObjVar(structure, "avatar_platform.wke_completed_02"))
            {
                ...
                obj_id cellDoor = getObjIdObjVar(structure, "avatar_platform.cell_door_02");
        ...
        if (item == menu_info_types.SERVER_MENU1)
        {
            ...
            if (!hasObjVar(structure, "avatar_platform.wke_completed_02"))
            {
                dictionary who = new dictionary();
                who.put("player", player);
                messageTo(target, "handleGasDeath", who, 0.0f, false);
