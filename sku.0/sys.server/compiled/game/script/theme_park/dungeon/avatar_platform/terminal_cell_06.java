...
public class terminal_cell_06 extends script.base_script
{
    public terminal_cell_06()
    {
    }
    ...
    public static final string_id EXECUTE = new string_id(STF, "error");
    ...
    public int OnObjectMenuSelect(obj_id self, obj_id player, int item) throws InterruptedException
    {
        if (item == menu_info_types.ITEM_USE)
        {
            ...
            if (!hasObjVar(structure, "avatar_platform.wke_completed_06"))
            {
        ...
        {
            obj_id structure = getTopMostContainer(self);
            obj_id target = getObjIdObjVar(structure, "avatar_platform.jawa_attacker");
            if (!hasObjVar(structure, "avatar_platform.wke_completed_06"))
            {
                dictionary info = new dictionary();
                info.put("player", player);
                messageTo(target, "handleSummonJawa", info, 3.0f, false);
