...
public class terminal_cell_01 extends script.base_script
{
    public terminal_cell_01()
    {
    }
    ...
    public int OnObjectMenuSelect(obj_id self, obj_id player, int item) throws InterruptedException
    {
        if (item == menu_info_types.ITEM_USE)
        {
            ...
            obj_id target = getObjIdObjVar(structure, "avatar_platform.wke_prisoner_01");
            if (!hasObjVar(structure, "avatar_platform.wke_completed_01"))
            {
                ...
                obj_id cellDoor = getObjIdObjVar(structure, "avatar_platform.cell_door_01");
        ...
        if (item == menu_info_types.SERVER_MENU1)
        {
            ...
            if (!hasObjVar(structure, "avatar_platform.wke_completed_01"))
            {
                dictionary who = new dictionary();
                who.put("player", player);
                messageTo(target, "handleFireDeath", who, 0.0f, false);
