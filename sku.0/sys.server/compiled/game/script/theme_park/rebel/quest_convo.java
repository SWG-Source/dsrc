...
public class quest_convo extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id speaker) throws InterruptedException
    {
        ...
        {
            debugSpeakMsg(self, "Sorry, only rebel employees are allowed to work for us.");
        ...
        if (gating <= 1)
        {
            if (!hasObjVar(speaker, "theme_park_reset.rebel"))
            {
                setObjVar(speaker, "theme_park_reset.rebel", true);
            }
        }
        else 
        {
            if (!hasObjVar(speaker, "theme_park_reset.rebel"))
            {
                ...
                if (loc.area.equals("corellia"))
                {
                    needReset = new string_id(MESSAGE_FILE, "need_reset_rebel");
                }
                else 
                {
                    needReset = new string_id(MESSAGE_FILE, "need_reset_rebel_offworld");
                }
        ...
        else if (hasObjVar(speaker, questID + ".done"))
        {
            ...
            if (type.equals("fetch") || type.equals("retrieve"))
            {
                ...
                if (!checkForItem(playerInv, speaker, questNum) == true)
                {
                    ...
                    String one = getString(response[0]);
                    String two = getString(response[1]);
                    if (one == null || one.equals(""))
                    {
                        one = "player_reset_" + questNum;
                        response[0] = new string_id(CONVO, one);
                    }
                    if (two == null || two.equals(""))
                    {
                        two = "player_sorry_" + questNum;
                        response[1] = new string_id(CONVO, two);
                    }
    ...
    public int OnNpcConversationResponse(obj_id self, String convo, obj_id player, string_id response) throws InterruptedException
    {
        ...
        String response8 = "player_sorry";
        String response9 = "player_reset";
        ...
        {
            String npcAnswer2 = "npc_backtowork_" + questNum;
            string_id message = new string_id(CONVO, npcAnswer2);
            npcSpeak(player, message);
            npcEndConversation(player);
            return SCRIPT_CONTINUE;
        ...
        {
            String npcAnswer2 = "npc_reset_" + questNum;
            string_id message = new string_id(CONVO, npcAnswer2);
            npcSpeak(player, message);
    ...
    public void giveReward(obj_id self, obj_id player, int questNum) throws InterruptedException
    {
        ...
        if (factionReward > 0)
        {
            factions.addFactionStanding(player, factions.FACTION_REBEL, (float)factionReward);
        }
        int credits = dataTableGetInt(datatable, questNum, "credits");
        if (credits != 0)
        {
            money.bankTo(money.ACCT_REBEL, player, credits);
