...
public class npc_mission_03_convo extends script.systems.missions.base.mission_dynamic_base
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id speaker) throws InterruptedException
    {
        ...
        if (objMission != null)
        {
            ...
            {
                string_id greeting = new string_id(CONVO, "npc_msn_03_greeting_onmission");
        ...
        else 
        {
            ...
            if (objMissionArray != null)
            {
                ...
                {
                    string_id greeting = new string_id(CONVO, "npc_msn_03_greeting_onothermission");
                    chat.chat(self, greeting);
                    return SCRIPT_CONTINUE;
                }
            }
        }
        string_id greeting = new string_id(CONVO, "npc_msn_03_greeting_01");
        string_id response[] = new string_id[2];
        response[0] = new string_id(CONVO, "npc_msn_03_presp_greeting_01");
        response[1] = new string_id(CONVO, "npc_msn_03_presp_greeting_02");
        npcStartConversation(speaker, self, "missionConvo", greeting, response);
        return SCRIPT_CONTINUE;
    }
    public int OnNpcConversationResponse(obj_id self, String convo, obj_id player, string_id response) throws InterruptedException
    {
        if ((response.getAsciiId()).equals("npc_msn_03_presp_greeting_01"))
        {
            string_id message = new string_id(CONVO, "npc_msn_03_work_01");
            npcSpeak(player, message);
            npcRemoveConversationResponse(player, new string_id(CONVO, "npc_msn_03_presp_greeting_01"));
            npcRemoveConversationResponse(player, new string_id(CONVO, "npc_msn_03_presp_greeting_02"));
            npcAddConversationResponse(player, new string_id(CONVO, "npc_msn_03_presp_getmission_03"));
            npcAddConversationResponse(player, new string_id(CONVO, "npc_msn_03_presp_getmission_04"));
            return SCRIPT_CONTINUE;
        }
        if ((response.getAsciiId()).equals("npc_msn_03_presp_greeting_02"))
        {
            string_id message = new string_id(CONVO, "npc_msn_03_busy_01");
            npcSpeak(player, message);
            npcRemoveConversationResponse(player, new string_id(CONVO, "npc_msn_03_presp_greeting_01"));
            npcRemoveConversationResponse(player, new string_id(CONVO, "npc_msn_03_presp_greeting_02"));
            npcAddConversationResponse(player, new string_id(CONVO, "npc_msn_03_presp_getmission_05"));
            return SCRIPT_CONTINUE;
        }
        if ((response.getAsciiId()).equals("npc_msn_03_presp_getmission_03") || (response.getAsciiId()).equals("npc_msn_03_presp_getmission_04"))
        {
            string_id message = new string_id(CONVO, "npc_msn_03_work_02");
            npcSpeak(player, message);
            npcRemoveConversationResponse(player, new string_id(CONVO, "npc_msn_03_presp_getmission_03"));
            npcRemoveConversationResponse(player, new string_id(CONVO, "npc_msn_03_presp_getmission_04"));
            npcAddConversationResponse(player, new string_id(CONVO, "npc_msn_03_presp_getmission_01"));
            npcAddConversationResponse(player, new string_id(CONVO, "npc_msn_03_presp_getmission_02"));
            return SCRIPT_CONTINUE;
        }
        if ((response.getAsciiId()).equals("npc_msn_03_presp_getmission_05"))
        {
            string_id message = new string_id(CONVO, "npc_msn_03_done_01");
            npcSpeak(player, message);
            npcEndConversation(player);
            return SCRIPT_CONTINUE;
        }
        if ((response.getAsciiId()).equals("npc_msn_03_presp_getmission_01") || (response.getAsciiId()).equals("npc_msn_03_presp_getmission_02"))
        {
            obj_id objMission = getAssignedNPCMission(self, player);
            if (objMission != null)
            {
                string_id strResponse = new string_id(CONVO, "npc_msn_03_greeting_onmission");
        ...
        {
            string_id strResponse = new string_id(CONVO, "npc_msn_03_decline");
            chat.chat(self, strResponse);
            npcEndConversation(player);
        }
        return SCRIPT_CONTINUE;
