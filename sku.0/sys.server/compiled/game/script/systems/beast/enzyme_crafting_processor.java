...
{
    public enzyme_crafting_processor()
    {
    }
    public int OnObjectMenuRequest(obj_id self, obj_id player, menu_info mi) throws InterruptedException
    {
        ...
        if (isInOperation())
        {
            ...
            mi.addSubMenu(options, menu_info_types.ITEM_DEACTIVATE, new string_id("beast", "stop_processor"));
        }
        if (isReadyToOperate())
        {
            int options = mi.addRootMenu(menu_info_types.ITEM_USE, new string_id("beast", "start_processor"));
            if (isGod(player))
            {
    ...
    public int OnObjectMenuSelect(obj_id self, obj_id player, int item) throws InterruptedException
    {
        ...
        if (item == menu_info_types.ITEM_USE)
        {
            ...
            pp = prose.setStringId(pp, new string_id("beast", "processor_runtime_info"));
            if (isInOperation())
            {
            ...
            else if (isReadyToOperate())
            {
                ...
                {
                    pp = prose.setTO(pp, utils.formatTimeVerbose(PROCESSOR_RUNTIME));
                    sendSystemMessageProse(player, pp);
                }
            }
        }
