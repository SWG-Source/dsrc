...
public class no_consume_loot extends script.base_script
{
    ...
    public static final string_id SID_NO_CONSUME_PROMPT = new string_id("collection", "no_consume_item_prompt");
    ...
    public int OnObjectMenuRequest(obj_id self, obj_id player, menu_info mi) throws InterruptedException
    {
        ...
        if (utils.isNestedWithinAPlayer(collectionItem) && !hasObjVar(collectionItem, OBJVAR_SLOT_GRANTED))
        {
    ...
    public int OnObjectMenuSelect(obj_id self, obj_id player, int item) throws InterruptedException
    {
        ...
        if (item == menu_info_types.SERVER_MENU3 && utils.isNestedWithinAPlayer(collectionItem))
        {
            ...
            if (hasObjVar(collectionItem, OBJVAR_SLOT_GRANTED))
            {
                sendSystemMessage(player, SID_ALREADY_BEEN_USED);
                return SCRIPT_CONTINUE;
            }
    ...
    public int OnGetAttributes(obj_id self, obj_id player, String[] names, String[] attribs) throws InterruptedException
    {
        ...
        if (hasObjVar(self, OBJVAR_SLOT_GRANTED))
        {
            return SCRIPT_CONTINUE;
        }
    ...
    public boolean getUiConsumeMessageBox(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        {
            int pid = sui.msgbox(self, player, "@" + SID_NO_CONSUME_PROMPT, sui.YES_NO, "@" + SID_CONSUME_TITLE, "handlerSuiAddToCollection");
            sui.setPid(player, pid, PID_NAME);
            return true;
        }
