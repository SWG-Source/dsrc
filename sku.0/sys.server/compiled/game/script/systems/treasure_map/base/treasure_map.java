...
public class treasure_map extends script.base_script
{
    ...
    public boolean storeWaypoint(obj_id map, obj_id player) throws InterruptedException
    {
        ...
        if (locationMapLocation != null && !locationMapLocation.equals(""))
        {
            ...
            else if (isGod(player))
            {
                ...
                CustomerServiceLog("treasureMap", "Player: " + player + " " + getName(player) + " is godplayer using map: " + map + " to make the map waypoint on their current planet location as opposed to: " + locationMapLocation + ". [ storeWaypoint() ]");
            }
    ...
    public void extractTreasure(obj_id map, obj_id player) throws InterruptedException
    {
        ...
        if (dist <= 8 && dist >= 0)
        {
            ...
            CustomerServiceLog("treasureMap", "Player: " + player + " " + getName(player) + " is extracting a chest using: " + map + " at location " + getLocation(player) + ". [ extractTreasure() ]");
            dictionary params = new dictionary();
            params.put("player", player);
    ...
    public int spawnTreasure(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        if (!paramsReceived)
        {
            CustomerServiceLog("treasureMap", "TREASURE MAP ERROR: Player: " + player + " " + getName(player) + " could not spawn a treasure chest at location: " + getLocation(player) + ". Please notify development. [ spawnTreasure() ]");
        ...
        {
            CustomerServiceLog("treasureMap", "TREASURE MAP ERROR: Player: " + player + " " + getName(player) + " could not spawn a treasure chest at location: " + playerLocation + ". Please notify development. [ spawnTreasure() ]");
            doDebugLogging("treasureMap", "a treasure map failed to spawn for player: " + player + ". [ spawnTreasure() ]");
            sendSystemMessage(player, "You could not extract the chest. Notify Customer Service.", null);
            return SCRIPT_CONTINUE;
        }
        ...
        if (!verifiedAllVars)
        {
            CustomerServiceLog("treasureMap", "TREASURE MAP ERROR: Player: " + player + " " + getName(player) + " could not spawn guards correctly because data taken off the map: " + map + " was incomplete, corrupted or invalid. [ spawnTreasure() ]");
        ...
        doDebugLogging("treasureMap", "loot_table: " + loot_table + " count: " + count + " type: " + type + "player: " + player + "treasureLoc: " + playerLocation + "boss_mob: " + boss_mob);
        CustomerServiceLog("treasureMap", "Player: " + player + " " + getName(player) + " spawned treasure chest: " + treasureChest + " at location: " + getLocation(treasureChest) + " with " + count + " mobs of type: " + type + " and their level being: " + mobLevel + ". [ spawnTreasure() ]");
        messageTo(treasureChest, "spawnTreasureGuards", outparams, 0, false);
        CustomerServiceLog("treasureMap", "Player: " + player + " " + getName(player) + " extracted the treasure chest at location: " + playerLocation + " and the map:" + map + " was destroyed. [ spawnTreasure() ]");
