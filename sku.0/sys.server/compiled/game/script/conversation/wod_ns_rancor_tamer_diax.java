...
public class wod_ns_rancor_tamer_diax extends script.base_script
{
    ...
    public boolean wod_ns_rancor_tamer_diax_condition_hasPreqComplete(obj_id player, obj_id npc) throws InterruptedException
    {
        ...
        {
            groundquests.clearQuest(player, "quest/wod_themepark_ns_rancor_tamer");
            return true;
        }
        return false;
    }
    ...
    public int wod_ns_rancor_tamer_diax_handleBranch4(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        ...
        {
            if (wod_ns_rancor_tamer_diax_condition__defaultCondition(player, npc))
            {
                wod_ns_rancor_tamer_diax_action_grantTPTamerNS(player, npc);
        ...
        if (response.equals("s_18"))
        {
            if (wod_ns_rancor_tamer_diax_condition__defaultCondition(player, npc))
            {
                ...
                if (hasResponse)
                {
                    ...
                    utils.setScriptVar(player, "conversation.wod_ns_rancor_tamer_diax.branchId", 9);
        ...
        if (response.equals("s_22"))
        {
            if (wod_ns_rancor_tamer_diax_condition__defaultCondition(player, npc))
            {
                ...
                if (hasResponse)
                {
                    ...
                    utils.setScriptVar(player, "conversation.wod_ns_rancor_tamer_diax.branchId", 10);
                    npcSpeak(player, message);
                    npcSetConversationResponses(player, responses);
                }
                else 
        ...
        if (response.equals("s_26"))
        {
            if (wod_ns_rancor_tamer_diax_condition__defaultCondition(player, npc))
            {
                ...
                if (hasResponse)
                {
                    ...
                    utils.setScriptVar(player, "conversation.wod_ns_rancor_tamer_diax.branchId", 11);
                    npcSpeak(player, message);
                    npcSetConversationResponses(player, responses);
                }
                else 
        ...
        if (response.equals("s_30"))
        {
            if (wod_ns_rancor_tamer_diax_condition__defaultCondition(player, npc))
            {
                ...
                if (hasResponse)
                {
                    ...
                    utils.setScriptVar(player, "conversation.wod_ns_rancor_tamer_diax.branchId", 12);
                    npcSpeak(player, message);
                    npcSetConversationResponses(player, responses);
                }
                else 
        ...
        if (response.equals("s_34"))
        {
            if (wod_ns_rancor_tamer_diax_condition__defaultCondition(player, npc))
            {
                ...
                if (hasResponse)
                {
                    ...
                    utils.setScriptVar(player, "conversation.wod_ns_rancor_tamer_diax.branchId", 13);
                    npcSpeak(player, message);
                    npcSetConversationResponses(player, responses);
                }
                else 
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (wod_ns_rancor_tamer_diax_condition_IsIndifferent(player, npc))
        {
            string_id message = new string_id(c_stringFile, "s_5");
            chat.chat(npc, player, message);
            return SCRIPT_CONTINUE;
        }
        if (wod_ns_rancor_tamer_diax_condition_IsSM(player, npc))
        {
            string_id message = new string_id(c_stringFile, "s_7");
            chat.chat(npc, player, message);
            return SCRIPT_CONTINUE;
        }
        if (wod_ns_rancor_tamer_diax_condition_IsNoEntertainer(player, npc))
        {
            string_id message = new string_id(c_stringFile, "s_9");
            chat.chat(npc, player, message);
            return SCRIPT_CONTINUE;
        }
        if (wod_ns_rancor_tamer_diax_condition_hasCompletedTamerNS(player, npc))
        {
            ...
            if (hasResponse)
            {
                ...
                utils.setScriptVar(player, "conversation.wod_ns_rancor_tamer_diax.branchId", 4);
        ...
        if (wod_ns_rancor_tamer_diax_condition_hasQuestActive(player, npc))
        {
            string_id message = new string_id(c_stringFile, "s_46");
            chat.chat(npc, player, message);
            return SCRIPT_CONTINUE;
        }
        if (wod_ns_rancor_tamer_diax_condition_hasPreqComplete(player, npc))
        {
            ...
            if (hasResponse)
            {
                ...
                utils.setScriptVar(player, "conversation.wod_ns_rancor_tamer_diax.branchId", 8);
                prose_package pp = new prose_package();
                pp.stringId = message;
                pp.actor.set(player);
                pp.target.set(npc);
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        obj_id npc = self;
        int branchId = utils.getIntScriptVar(player, "conversation.wod_ns_rancor_tamer_diax.branchId");
        if (branchId == 4 && wod_ns_rancor_tamer_diax_handleBranch4(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 8 && wod_ns_rancor_tamer_diax_handleBranch8(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 9 && wod_ns_rancor_tamer_diax_handleBranch9(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 10 && wod_ns_rancor_tamer_diax_handleBranch10(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 11 && wod_ns_rancor_tamer_diax_handleBranch11(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 12 && wod_ns_rancor_tamer_diax_handleBranch12(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 13 && wod_ns_rancor_tamer_diax_handleBranch13(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
