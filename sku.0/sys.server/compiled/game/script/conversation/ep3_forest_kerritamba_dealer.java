...
public class ep3_forest_kerritamba_dealer extends script.base_script
{
    ...
    public boolean ep3_forest_kerritamba_dealer_condition_check_inv(obj_id player, obj_id npc) throws InterruptedException
    {
    ...
    public void ep3_forest_kerritamba_dealer_action_start_dealing(obj_id player, obj_id npc) throws InterruptedException
    {
        dictionary params = new dictionary();
        params.put("player", player);
        messageTo(npc, "startDealing", params, 1.0f, false);
    }
    public int ep3_forest_kerritamba_dealer_handleBranch1(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("s_c20cb2b0"))
        {
            if (ep3_forest_kerritamba_dealer_condition__defaultCondition(player, npc))
            {
                string_id message = new string_id(c_stringFile, "s_47a68e94");
                int numberOfResponses = 0;
                boolean hasResponse = false;
                boolean hasResponse0 = false;
                if (ep3_forest_kerritamba_dealer_condition_check_inv(player, npc))
                {
                    ++numberOfResponses;
                    hasResponse = true;
                    hasResponse0 = true;
                ...
                if (hasResponse)
                {
                    ...
                    utils.setScriptVar(player, "conversation.ep3_forest_kerritamba_dealer.branchId", 2);
                    npcSpeak(player, message);
                    npcSetConversationResponses(player, responses);
                }
                else 
                {
                    utils.removeScriptVar(player, "conversation.ep3_forest_kerritamba_dealer.branchId");
    ...
    public int ep3_forest_kerritamba_dealer_handleBranch2(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("s_e022ccee"))
        {
            if (ep3_forest_kerritamba_dealer_condition__defaultCondition(player, npc))
            {
                ep3_forest_kerritamba_dealer_action_start_dealing(player, npc);
    ...
    public int OnInitialize(obj_id self) throws InterruptedException
    {
        ...
        {
            detachScript(self, "conversation.ep3_forest_kerritamba_dealer");
        }
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (ep3_forest_kerritamba_dealer_condition__defaultCondition(player, npc))
        {
            ...
            if (hasResponse)
            {
                ...
                utils.setScriptVar(player, "conversation.ep3_forest_kerritamba_dealer.branchId", 1);
                npcStartConversation(player, npc, "ep3_forest_kerritamba_dealer", message, responses);
            }
            else 
            {
                chat.chat(npc, player, message);
    ...
    {
        if (!conversationId.equals("ep3_forest_kerritamba_dealer"))
        {
            return SCRIPT_CONTINUE;
        }
        obj_id npc = self;
        int branchId = utils.getIntScriptVar(player, "conversation.ep3_forest_kerritamba_dealer.branchId");
        if (branchId == 1 && ep3_forest_kerritamba_dealer_handleBranch1(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 2 && ep3_forest_kerritamba_dealer_handleBranch2(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
