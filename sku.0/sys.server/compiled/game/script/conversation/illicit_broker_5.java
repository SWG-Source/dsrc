...
public class illicit_broker_5 extends script.base_script
{
    ...
    public void illicit_broker_5_action_startGiveQuest(obj_id player, obj_id npc) throws InterruptedException
    {
        ...
        obj_id[] missionContraband = utils.getAllStaticItemsInPlayerInventory(player, illicit_broker_5_tokenTO_contrabandName);
        ...
        if (contrabandSize >= 5)
        {
            ...
            CustomerServiceLog("SMUGGLER_SYSTEM", player + "Received Tier 5 Illicit Contraband mission from " + npc);
    ...
    public void illicit_broker_5_action_sellContraband(obj_id player, obj_id npc) throws InterruptedException
    {
        String illicit_broker_5_tokenTO_contrabandName = utils.getStringScriptVar(player, "illicitContrabandTrade");
        obj_id pInv = utils.getInventoryContainer(player);
        obj_id[] tradeContraband = utils.getAllStaticItemsInPlayerInventory(player, illicit_broker_5_tokenTO_contrabandName);
        ...
        if (contrabandSize >= 5)
        {
            ...
            money.systemPayout(money.ACCT_RELIC_DEALER, player, smuggler.TIER_5_ILLICIT_SELL_VALUE, null, params);
            utils.removeScriptVar(player, "illicitContrabandTrade");
        }
    }
    ...
    public int illicit_broker_5_handleBranch5(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        ...
        {
            if (illicit_broker_5_condition__defaultCondition(player, npc))
            {
                ...
                if (hasResponse)
                {
                    ...
                    utils.setScriptVar(player, "conversation.illicit_broker_5.branchId", 6);
                    prose_package pp = new prose_package();
                    pp.stringId = message;
                    pp.actor.set(player);
                    pp.target.set(npc);
                    pp.other.set(illicit_broker_5_tokenTO_contrabandName(player, npc));
                    pp.digitInteger = illicit_broker_5_tokenDI_cashValue(player, npc);
                    npcSpeak(player, pp);
        ...
        if (response.equals("s_53"))
        {
            if (illicit_broker_5_condition__defaultCondition(player, npc))
            {
                ...
                if (hasResponse)
                {
                    ...
                    utils.setScriptVar(player, "conversation.illicit_broker_5.branchId", 9);
        ...
        {
            if (illicit_broker_5_condition_smugglerAboveMissionTier(player, npc))
            {
                ...
                boolean hasResponse1 = false;
                if (illicit_broker_5_condition_hasMissionContrabandToDeliver(player, npc))
                {
                    ++numberOfResponses;
                    hasResponse = true;
                    hasResponse1 = true;
                ...
                if (hasResponse)
                {
                    ...
                    utils.setScriptVar(player, "conversation.illicit_broker_5.branchId", 5);
                    npcSpeak(player, message);
                    npcSetConversationResponses(player, responses);
                }
                else 
    ...
    public int illicit_broker_5_handleBranch6(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        ...
        {
            illicit_broker_5_action_sellContraband(player, npc);
            if (illicit_broker_5_condition__defaultCondition(player, npc))
            {
    ...
    public int illicit_broker_5_handleBranch9(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        ...
        {
            illicit_broker_5_action_startGiveQuest(player, npc);
            if (illicit_broker_5_condition__defaultCondition(player, npc))
            {
    ...
    public int illicit_broker_5_handleBranch14(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        ...
        {
            illicit_broker_5_action_giveIllicitPointerQuest(player, npc);
            if (illicit_broker_5_condition__defaultCondition(player, npc))
            {
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (illicit_broker_5_condition_isNonSmuggler(player, npc))
        {
            string_id message = new string_id(c_stringFile, "s_4");
            chat.chat(npc, player, message);
            return SCRIPT_CONTINUE;
        }
        if (illicit_broker_5_condition_isDoingMission(player, npc))
        {
            string_id message = new string_id(c_stringFile, "s_6");
            chat.chat(npc, player, message);
            return SCRIPT_CONTINUE;
        }
        if (!illicit_broker_5_condition_hasSomeContraband(player, npc))
        {
            string_id message = new string_id(c_stringFile, "s_49");
            chat.chat(npc, player, message);
            return SCRIPT_CONTINUE;
        }
        if (!illicit_broker_5_condition_hasMissionContraband(player, npc))
        {
            string_id message = new string_id(c_stringFile, "s_50");
            chat.chat(npc, player, message);
            return SCRIPT_CONTINUE;
        }
        if (illicit_broker_5_condition_hasMissionContrabandBusiness(player, npc))
        {
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        obj_id npc = self;
        int branchId = utils.getIntScriptVar(player, "conversation.illicit_broker_5.branchId");
        if (branchId == 5 && illicit_broker_5_handleBranch5(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 6 && illicit_broker_5_handleBranch6(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 9 && illicit_broker_5_handleBranch9(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 12 && illicit_broker_5_handleBranch12(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 13 && illicit_broker_5_handleBranch13(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 14 && illicit_broker_5_handleBranch14(player, npc, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
