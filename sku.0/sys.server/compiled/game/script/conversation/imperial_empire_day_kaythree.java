...
public class imperial_empire_day_kaythree extends script.base_script
{
    ...
    public boolean imperial_empire_day_kaythree_condition_isNeutralWorkingForRebel(obj_id player, obj_id npc) throws InterruptedException
    {
        ...
        {
            return (((getCollectionSlotValue(player, holiday.REBEL_ENGINEERING_START_SLOT) > 0) && !hasCompletedCollection(player, holiday.REBEL_ENGINEERING_COLLECTION)) || ((getCollectionSlotValue(player, holiday.REBEL_ENTERTAINER_START_SLOT) > 0) && !hasCompletedCollection(player, holiday.REBEL_ENTERTAINER_COLLECTION)));
        }
        return false;
    }
    ...
    {
        faceTo(npc, player);
        if ((imperial_empire_day_kaythree_condition_isEntertainer(player, npc) && groundquests.hasCompletedQuest(player, holiday.EMPIREDAYQUEST_IMP_ENTERTAINER_06)))
        {
            return true;
        }
