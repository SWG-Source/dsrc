...
public class fs_grant_assembly_10 extends script.base_script
{
    public fs_grant_assembly_10()
    {
    }
    ...
    public int OnAttach(obj_id self) throws InterruptedException
    {
        ...
        if (isIdValid(player))
        {
            ...
            if (row >= 0)
            {
                ...
                {
                    CustomerServiceLog("fs_village", "WARNING: fs_grant_assembly_10 could not find xp data for skill " + SKILL + " in the skills datatable");
                }
            }
            else 
            {
                CustomerServiceLog("fs_village", "WARNING: fs_grant_assembly_10 could not find skill " + SKILL + " in the skills datatable");
            }
        }
        else 
        {
