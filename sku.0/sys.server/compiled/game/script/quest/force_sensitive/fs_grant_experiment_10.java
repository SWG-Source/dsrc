...
public class fs_grant_experiment_10 extends script.base_script
{
    ...
    public int OnAttach(obj_id self) throws InterruptedException
    {
        ...
        if (isIdValid(player))
        {
            ...
            if (row >= 0)
            {
                ...
                {
                    CustomerServiceLog("fs_village", "WARNING: fs_grant_experiment_10 could not find xp data for skill " + SKILL + " in the skills datatable");
                }
            }
            else 
            {
                CustomerServiceLog("fs_village", "WARNING: fs_grant_experiment_10 could not find skill " + SKILL + " in the skills datatable");
            }
        }
        else 
        {
            CustomerServiceLog("fs_village", "WARNING: fs_grant_experiment_10 could not find player containing object " + self);
        }
    ...
    {
        detachScript(self, "quest.force_sensitive.fs_grant_experiment_10");
        return SCRIPT_CONTINUE;
    }
}
